...
def create_dataset_buffer(cmp, device, times, dim_datasamples, list_G, mixed_rollout='False', show_graphs_stats=True):
    ...
    for G in list_G:
        if show_graphs_stats:
            # Calculate density for PyTorch Geometric Data objects
            num_edges = G.edge_index.size(1)
            num_nodes = G.num_nodes
            if num_nodes > 1:
                density = (num_edges / (num_nodes * (num_nodes - 1))) * 2
            else:
                density = 0  # Handle edge case where the graph has one or zero nodes
            print(f'Density: {density:<.3f}', G, sep=' | ')

        # Convert PyTorch Geometric Data to NetworkX graph for compatibility with other functions
        nx_graph = to_networkx(G, to_undirected=True)
        list_graphs, val = build_tree(nx_graph, cmp, device)
        list_val.append(val)
